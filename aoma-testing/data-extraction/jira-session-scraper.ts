...
class JiraSessionScraper {
  ...
  async testAPIAccess() {
    ...
    const testUrl = 'https://jirauat.smedigitalapps.com/jira/rest/api/2/search?jql=key=ITSM-55362&maxResults=1';
  ...
  async fetchTickets(jql: string = 'ORDER BY created DESC', maxResults: number = 10) {
    ...
    const url = `https://jirauat.smedigitalapps.com/jira/rest/api/2/search?jql=${encodeURIComponent(jql)}&maxResults=${maxResults}&fields=key,summary,created,status,priority,description`;
