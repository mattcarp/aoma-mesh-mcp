  ...
  try {
    ...
    for (const url of directUrls) {
      try {
        ...
        await page.goto(`https://jirauat.smedigitalapps.com/jira${url}`);
        await page.waitForTimeout(3000);
...
async function extractAndTestSession(page: any) {
  ...
  const response = await fetch('https://jirauat.smedigitalapps.com/jira/rest/api/2/search?jql=ORDER BY created DESC&maxResults=10&fields=key,summary,created,status', {
    headers: {
      'Cookie': cookieHeader,
      'Accept': 'application/json'
    }
