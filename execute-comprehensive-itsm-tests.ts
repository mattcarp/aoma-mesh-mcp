...
class ITSMTestRunner {
  ...
  async executeComprehensiveTests() {
    console.log('🦁 EXECUTING COMPREHENSIVE ITSM TEST SUITE');
    console.log('============================================');
  ...
  async testITSMDashboard(page: Page) {
    ...
    try {
      ...
      const status = loadTime < 10000 ? 'PASS' : 'WARN';
  ...
  async testITSMIssueNavigator(page: Page) {
    ...
    try {
      ...
      const status = isWorking && loadTime < 15000 ? 'PASS' : 'WARN';
  ...
  async testITSMProjectBrowser(page: Page) {
    ...
    try {
      ...
      let status = loginRequired ? 'FAIL' : (itsmVisible ? 'PASS' : 'WARN');
  ...
  async performanceValidation(page: Page) {
    ...
    for (const url of testUrls) {
      ...
      try {
        ...
        this.addResult(`PERF_${url.split('/').pop()}`, 'FAIL', Date.now() - start,
  ...
  async testTicketCreation(page: Page) {
    ...
    try {
      ...
      let status = hasError ? 'FAIL' : hasCreateForm ? 'PASS' : 'WARN';
...
runner.executeComprehensiveTests().catch(console.error); 